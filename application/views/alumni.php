...
<div class="container">
          ...
          <div class="modal-body">
            <form id="form-alumni" name="form-alumni">
              ...
              <div class="form-group">
                ...
                <select name="id_program_studi" id="id_program_studi" class="form-control">
                </select>
              </div>
  ...
  <script type="text/javascript">
    $(document).ready(function(){
        ...
        $.ajax({
                method: "POST",
                url: "<?php echo base_url();?>index.php/Oprec/getListProdi/",
                data: {}
              }).done(function( res ) {
                var prodi = JSON.parse(res);
                $.each(prodi, function(key,value){
                  $('#id_program_studi').append($("<option></option>").attr("value",value['id_program_studi']).text(`${value['id_program_studi']} : ${value['nama_prodi']}`));
              });
        });
        ...
        $("#btn-tambah-alumni").click(function(){
              ...
              $("#tanggal_lahir").val('');
              $("#email").val('');
              $('#id_program_studi').append($("<option><option>").attr("value",0).text("Pilih Program Studi"));
              const defaultValue =0;
              $('#id_program_studi').val(defaultValue).find(`option[value=${defaultValue}]`).attr('selected',true);
        });
    });
  </script>